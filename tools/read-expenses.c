/* read-expenses.c: Sample code to translate Palm Expense database into generic format
 *
...
int main(int argc, char *argv[])
{
   ...
   ret =
       dlp_ReadAppPreference(sd, Expense_Creator, Expense_Pref, 1, 0xffff,
			     buffer, 0, 0);
   ...
   for (i = 0;; i++) {
	 ...
	 continue;
      ...
      fprintf(stderr, "Type: %d, Payment: %d, Currency: %d\n", t.type,
	      t.payment, t.currency);
      fprintf(stderr, "Amount: '%s', Vendor: '%s', City: '%s'\n",
	      t.amount ? t.amount : "<None>",
	      t.vendor ? t.vendor : "<None>", t.city ? t.city : "<None>");
