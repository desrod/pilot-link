...
int main(int argc, char *argv[])
{
   ...
   for (i = 2; i < argc; i++) {
	 ...
	 if (*cPtr == '\t') {
	    if (fieldno >= 4) {
	       if (fieldno == 4)
		  fprintf(stderr, "Too many fields on the line : %s\n",
			  fields[fieldno - 1]);
	       fieldno++;
	       continue;
	    }
	    ...
	    *cPtr++ = '\0';
	    if (fieldno != 4) {
	       fprintf(stderr, "Too few fields : %s", fields[0]);
	       fieldno = 0;
	       fields[fieldno++] = cPtr;
	       continue;
	    }
	    ...
	    Appointment_size =
		pack_Appointment(&appointment, Appointment_buf,
				 sizeof(Appointment_buf));
	    printf("desc: %s\n", appointment.description);
	    dlp_WriteRecord(sd, db, 0, 0, 0, Appointment_buf,
			    Appointment_size, 0);
	    fields[fieldno++] = cPtr;
	 } else {
	    cPtr++;
