...
extern "C"
{
   ...
   extern int syspkt_rx PI_ARGS((struct pi_socket * ps, void *buf, int len));

   extern int sys_Continue
      PI_ARGS((int sd, struct Pilot_registers * r, struct Pilot_watch * w));
   extern int sys_Step PI_ARGS((int sd));

   extern int sys_QueryState PI_ARGS((int sd));
   extern int sys_ReadMemory
      PI_ARGS((int sd, unsigned long addr, unsigned long len, void *buf));
   ...
   extern int sys_Find
      PI_ARGS(
	      (int sd, unsigned long startaddr, unsigned long stopaddr,
	       int len, int caseinsensitive, void *data,
	       unsigned long *found));

   extern int sys_RemoteEvent
      PI_ARGS(
	      (int sd, int penDown, int x, int y, int keypressed, int keymod,
	       int keyasc, int keycode));

   extern int sys_RPC
      PI_ARGS(
	      (int sd, int socket, int trap, long *D0, long *A0, int params,
	       struct RPC_param * param, int rep));
...
#define RPC_NoReply 0
   ...
   extern unsigned long DoRPC
      PI_ARGS((int sd, int socket, struct RPC_params * p, int *error));
