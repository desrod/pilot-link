...
pi_file_open(char *name)
{
   ...
   if (pf->nentries) {
	 ...
	 if (pf->resource_flag) {
	    ...
	    printf("Entry %d 0x%8.8X %2.2X @%X\n", i, (int) entp->uid,
		   entp->attrs, entp->offset);
#endif
	 }
      }

      for (i = 0, entp = pf->entries + pf->nentries - 1; i < pf->nentries;
	   i++, entp--) {
   ...
   if (pf->app_info_size == 0)
	 ...
	 goto bad;
	  ...
	  (size_t) pf->app_info_size) goto bad;
   }
...
pi_file_set_info(struct pi_file *pf, struct DBInfo *ip)
{
   ...
   if ((ip->flags & dlpDBFlagResource) !=
       (pf->info.flags & dlpDBFlagResource)) return (-1);
   ...
   if (pf->info.flags & dlpDBFlagResource)
	 ...
	 if (
	     (dlp_ReadResourceByIndex
	      (socket, db, j, buffer, &type, &id, &size) < 0)
	 ...
	 if (
	     (dlp_ReadRecordByIndex
	      (socket, db, j, buffer, &id, &size, &attr, &category) < 0)) {
	    dlp_CloseDB(socket, db);
	    return -1;
	 }
...
pi_file_merge(struct pi_file *pf, int socket, int cardno)
{
   ...
   if (dlp_OpenDB
       (socket, cardno, dlpOpenReadWrite | dlpOpenSecret, pf->info.name,
	&db) < 0)
