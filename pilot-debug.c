...
int
SetBreakpoint(int bp, unsigned long address, int enabled)
{
...
DbgGetDeviceVersion(void)
{
   ...
   PackRPC(&p, 0xA27B, RPC_IntReply,
	   RPC_Long(makelong("psys")), RPC_Short(1), RPC_LongPtr(&ROMversion),
	   RPC_End);
   ...
   buildVersion =
      (((ROMversion >> 8) & 0xf) * 10) + (((ROMversion >> 4) & 0xf) * 10) +
      (ROMversion & 0xf);
}
...
int proc_attach(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
   ...
   if (!debugger && !console) {
      Say
	 ("Unable to attach to to Palm. Is the Palm connected and in debugging or console mode?\n");
   }
...
int proc_coldboot(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
   ...
   PackRPC(&p, 0xA08B, RPC_NoReply, RPC_Long(0), RPC_Long(0), RPC_Long(0),
	   RPC_Long(0), RPC_End);
   DbgRPC(&p, 0);
...
/* printf("Volts = %f, crit = %f, warn = %f, ticks = %d, kind =
   ...
   if (usetk)
      Tcl_VarEval(interp, ".state.battery configure -fg ",
		  (v <= critical) ? "red" : (v <= warn) ? "darkred" : "blue",
		  NULL);
#endif
...
int proc_mirror(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
   ...
   PackRPC(&p, 0xA026, RPC_IntReply, RPC_LongPtr(&addr), RPC_Long(scrGlobals),
	   RPC_Long(4), RPC_End);
   e1 = DbgRPC(&p, &e2);
...
int proc_getdisplay(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
#ifndef TK
   Say
      ("getdisplay is not available due to pilot-debug being compiled without Tk support");
   return TCL_ERROR;
   ...
   if (debugger) {
	 ...
	 PackRPC(&p, 0xA026, RPC_IntReply, RPC_Ptr(buffer + l, 128),
		 RPC_Long(addr + l), RPC_Long(128), RPC_End);
	 e1 = DbgRPC(&p, &e2);
      }
   }
...
int proc_feature(ClientData clientData, Tcl_Interp *interp, int argc, char *argv[])
{
   ...
   if (strcmp(argv[1], "-all") == 0) {
	 ...
	 for (i = 0;; i++) {
	    ...
	    PackRPC(&p, 0xA27D, RPC_IntReply, RPC_Short(i), RPC_Byte(j),
		    RPC_LongPtr(&type), RPC_ShortPtr(&id),
		    RPC_LongPtr(&value), RPC_End);
	    e1 = DbgRPC(&p, &e2);
	    if (e1 || e2)
	       break;
	    sprintf(buffer, "\t%s, 0x%4.4x (%d) = 0x%8.8lx (%lu)",
		    printlong(type), id, id, value, value);
	    Say(buffer);
	 }
      }
   }
