...
static int
pi_serial_listen(struct pi_socket *ps, int backlog)
{
...
static int
pi_serial_send(struct pi_socket *ps, void *msg, int len, unsigned int flags)
{
   if (ps->type == PI_SOCK_STREAM)
...
static int
pi_serial_recv(struct pi_socket *ps, void *msg, int len, unsigned int flags)
{
   if (ps->type == PI_SOCK_STREAM)
...
static int
pi_serial_tickle(struct pi_socket *ps)
{
   if (ps->type == PI_SOCK_STREAM) {
      struct padp pd;
      int ret;
...
static int
pi_serial_close(struct pi_socket *ps)
{
#ifdef DEBUG
...
int
pi_serial_flush(struct pi_socket *ps)
{
