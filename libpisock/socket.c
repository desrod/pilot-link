...
alarm(unsigned sec)
{
   ...
   if (sec) {
	 ...
	 alm_tid =
	    CreateThread(0, 0, (LPTHREAD_START_ROUTINE) alarm_thread, 0, 0,
			 &t);
      }
   }
   else {
...
static int
default_socket_connect(struct pi_socket *ps, struct sockaddr *addr, int flag)
{
   errno = ENOSYS;
   return -1;
}
static int
default_socket_listen(struct pi_socket *ps, int flag)
{
   errno = ENOSYS;
   return -1;
}
static int
default_socket_accept(struct pi_socket *ps, struct sockaddr *addr, int *flag)
{
   errno = ENOSYS;
   return -1;
}
static int
default_socket_close(struct pi_socket *ps)
{
   return 0;
}
static int
default_socket_tickle(struct pi_socket *ps)
{
   errno = ENOSYS;
   return -1;
}
...
void
pi_socket_recognize(struct pi_socket *ps)
{
   struct pi_socket *p;
...
int
pi_accept(int pi_sd, struct sockaddr *addr, int *addrlen)
{
   struct pi_socket *ps;
...
int
pi_setmaxspeed(int pi_sd, int speed, int overclock)
{
   struct pi_socket *ps;
...
int
pi_send(int pi_sd, void *msg, int len, unsigned int flags)
{
   struct pi_socket *ps;
...
int
pi_recv(int pi_sd, void *msg, int len, unsigned int flags)
{
   struct pi_socket *ps;
...
int
pi_read(int pi_sd, void *msg, int len)
{
...
int
pi_write(int pi_sd, void *msg, int len)
{
...
int
pi_getsockname(int pi_sd, struct sockaddr *addr, int *namelen)
{
   struct pi_socket *ps;
...
int
pi_getsockpeer(int pi_sd, struct sockaddr *addr, int *namelen)
{
   struct pi_socket *ps;
