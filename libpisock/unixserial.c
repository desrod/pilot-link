...
#else
static int
cfsetspeed(struct termios *t, int speed)
{
...
static int
s_changebaud(struct pi_socket *ps)
{
#ifndef SGTTY
   struct termios tcn;
...
static int
s_read(struct pi_socket *ps, int timeout)
{
   int r;
   unsigned char *buf;
