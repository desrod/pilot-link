...
void Backup(char *dirname, int only_changed, int remove_deleted, int rom, int quiet, int do_rom, int unsaved, char *archive_dir)
{
   ...
   if (orig_files) {
	 ...
	 fprintf(stderr, "Exiting on error, stopped before removing files.\n");
	 exit(1);
      }
...
void Restore(char *dirname)
{
   ...
   if (!db) {
      fprintf(stderr, "Unable to allocate memory for directory entry table\n");
      exit(1);
   }
...
{
   PalmHeader(progname);
   fprintf(stderr, "   Type 'man 7 pilot-link' at your shell for more information.\n\n");
   exit(0);
}
