...
static const char *const yytname[] =
    { "$", "error", "$undefined.", "tDAY", "tDAYZONE",
	"tMERIDIAN", "tMONTH", "tMONTH_UNIT", "tSEC_UNIT", "tSNUMBER",
	    "tUNUMBER", "tZONE",
	"':'", "'/'", "','", "spec", "item", "time", "zone", "numzone",
	    "date", "rel", "o_merid", NULL
};
#endif
...
#endif
		yyss =
		    (short *) YYSTACK_ALLOC(yystacksize * sizeof(*yyssp));
		__yy_memcpy((char *) yyss, (char *) yyss1,
			    size * (unsigned int) sizeof(*yyssp));
		yyvs =
		    (YYSTYPE *) YYSTACK_ALLOC(yystacksize *
					      sizeof(*yyvsp));
		__yy_memcpy((char *) yyvs, (char *) yyvs1,
			    size * (unsigned int) sizeof(*yyvsp));
#ifdef YYLSP_NEEDED
		yyls =
		    (YYLTYPE *) YYSTACK_ALLOC(yystacksize *
					      sizeof(*yylsp));
...
#line 203 "parsedate.y"
	{
		...
		if (yyvsp[0].Number < 0) {
			...
			if (yyvsp[0].Number > 9999
			    || (i = yyvsp[0].Number % 100) >= 60) {
				YYABORT;
			}
...
static time_t ToSeconds(Hours, Minutes, Seconds, Meridian)
time_t Hours;
time_t Minutes;
time_t Seconds;
MERIDIAN Meridian;
...
DSTMODE dst;
{
	...
	if (Year < EPOCH || Year > END_OF_TIME || Month < 1 || Month > 12
...
static time_t DSTcorrect(Start, Future)
time_t Start;
time_t Future;
{
	time_t StartDay;
	...
	return (Future - Start) + (StartDay -
				   FutureDay) * DST_OFFSET * 60L * 60L;
}


static time_t RelativeMonth(Start, RelMonth)
time_t Start;
time_t RelMonth;
{
	struct tm *tm;
	...
	return DSTcorrect(Start,
			  Convert(Month, (time_t) tm->tm_mday, Year,
				  (time_t) tm->tm_hour,
				  (time_t) tm->tm_min, (time_t) tm->tm_sec,
				  MER24, DSTmaybe));
}


static int LookupWord(buff, length)
char *buff;
register int length;
{
	register char *p;
	...
	*p = '\0';
	...
	if (p - buff != length) {
		c = buff[0];
		for (p = buff, tp = TimezoneTable;
		     tp < ENDOF(TimezoneTable); tp++)
			if (c == tp->name[0] && p[1] == tp->name[1]
			    && strcmp(p, tp->name) == 0) {
				yylval.Number = tp->value;
				return tp->type;
...
char *p;
{
	...
	if (yyHaveDate || yyHaveTime) {
		Start =
		    Convert(yyMonth, yyDay, yyYear, yyHour, yyMinutes,
