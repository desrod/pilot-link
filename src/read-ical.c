...
struct option options[] = {
        ...
        {"dbonly",      no_argument,       NULL, 'd'},
        {"ptext",       required_argument, NULL, 't'},
        {"icalfile",    required_argument, NULL, 'f'},
        {NULL,          0,                 NULL, 0}
};
...
int main(int argc, char *argv[])
{
	...
	if (port == NULL && argc > 1) {
		...
		printf("Now removing existing file %s\n", icalfile);
		unlink(icalfile);

		/* Probably should test if 'ical' is on the path with fopen() first */
		sprintf(cmd, "ical -f - -calendar %s", icalfile);
		ical = popen(cmd, "w");
