...
#define MAX_PHOTO_NAME 30
   for (i=0; install_files[i]; i++) {
      strncpy(installed_name, install_files[i], MAX_PHOTO_NAME);
      installed_name[MAX_PHOTO_NAME]='\0';
      /* todo check maximum file name length */
      if (strlen(install_files[i]) > MAX_PHOTO_NAME - 3) {
	 strcpy(&installed_name[MAX_PHOTO_NAME - 3], ".jpg");
	 printf("%s being truncated to %s: ", install_files[i], installed_name);
      }
	 ...
	 printf("version %d.%02d\n", (version&0xFF00)>>8, version&0xFF);
	 printf("   Image dimensions (width x height): %d x %d", width, height);
      } else {
	...
	pdb_size = sbuf.st_size;
	 ...
	 return -1;
      ...
      ret = install_tux_with_name(sd, installed_name,
	      width, height, pdb_size, mod_time, file_time);
      if (ret<0) {
