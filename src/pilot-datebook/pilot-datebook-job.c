 ...
 * Function:    job_init
 *
 * Summary:     Initiate job data structures
 *
 * Parameters:  None
 *
 * Return:      Nothing
...
int job_set_param(struct job *job, char opt, char *opt_arg)
{
	...
	switch (job->type) {
	  case COMPARE_JOB:
		  rc = compare_set_param(&(job->job_data.compare), opt,
					 opt_arg);
		  break;
	  case READ_JOB:
		  rc = io_set_read_option(&(job->job_data.io), opt,
					  opt_arg);
		  break;
	  case WRITE_JOB:
		  rc = io_set_write_option(&(job->job_data.io), opt,
					   opt_arg);
		  break;
	  case SORT_JOB:
		  ...
		  break;
	  case UPDATE_JOB:
		  rc = update_set_param(&(job->job_data.update), opt,
					opt_arg);
		  break;
	  case DEL_JOB:
 ...
 * Function:    job_start
 *
 * Summary:     Start job, process header
 *
 * Parameters:  None
 *
 * Return:      Nothing
 ...
 *
 * Summary:     Work on job for one row
 *
 * Parameters:  None
 *
 * Return:      Nothing
 ...
 ***********************************************************************/
void
job_process(struct job *job, struct header_data *header,
	    struct row_data *row)
{
	int rc = FALSE;
	...
	switch (job->type) {
	  case COMPARE_JOB:
		  ...
		  if (!getRowIsValid(row)) {
			  ...
			  if (getRowIsSelected(row) != DATEBOOK_SELECT_YES)
				  compare_row(&(job->job_data.compare),
					      row);
	  ...
	  case WRITE_JOB:
		  ...
		  if (getRowIsValid(row)) {
			  /* Inform user */
			  info_message("Writing output record #%d\r",
				       job->record_num);
			  debug_message("\n");

			  /* Write row to output file */
			  if (getRowIsSelected(row) != DATEBOOK_SELECT_NO)
				  io_write_row(&(job->job_data.io), header,
					       row);
	  ...
	  case SORT_JOB:
		  ...
		  if (getRowIsValid(row)) {
			  /* Inform user */
			  info_message("Sorting record #%d\r",
				       job->record_num);
			  debug_message("\n");
			  ...
			  sort_write_row(&(job->job_data.sort), header,
					 row);
			  ...
			  if (!rc)
				  sort_read_row(&(job->job_data.sort),
						row);
		  }
		  break;
	  ...
	  case DEL_JOB:
		  ...
		  if (!getRowIsValid(row)) {
			  ...
			  if (getRowIsSelected(row) != DATEBOOK_SELECT_NO) {
				  ...
				  /* Update selection */
				  setRowIsSelected(row,
						   DATEBOOK_SELECT_UNKNOWN);
			  }
 ...
 * Function:    job_stop
 *
 * Summary:     Stop job at end of processing
 *
 * Parameters:  None
 *
 * Return:      Nothing
 ...
 * Function:    job_exit
 *
 * Summary:     Destroy job data structure
 *
 * Parameters:  None
 *
 * Return:      Nothing
 ...
 * Function:    job_abort
 *
 * Summary:     Stop job in case of error 
 *
 * Parameters:  None
 *
 * Return:      Nothing
