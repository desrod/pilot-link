...
void WritePicture( int sd, int db, char *name )
{
   ...
   if( f ) 
	...
	if( sd )
	  {
	     len = dlp_ReadRecordByIndex(sd, db, 0, inBuf, 0, 0, &attr, &category);
...
//		unpack_VeoAppInfo( &vai, buffer, 0xffff);
   ...
   if( sd )
     {
	dlp_AddSyncLogEntry( sd, "Successfully read Veo photos from Palm.\n"
